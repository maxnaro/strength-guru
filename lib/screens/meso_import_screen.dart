...
class MesoImportScreen extends ConsumerStatefulWidget {
  ...
  final Future<MesoImportData> Function(void Function(int, int)? onProgress)? dataProducer;
...
class _MesoImportScreenState extends ConsumerState<MesoImportScreen> {
  ...
  Future<void> _runProducer() async {
    ...
    try {
      final data = await widget.dataProducer!((done, total) {
        if (mounted) setState(() => _loadingLabel = 'Generating day $done/$total…');
      });
      await _handleData(data, emptyMessage: 'No training days were generated. Refine your description and try again.');
    } on LlmException catch (e) {
      if (mounted) setState(() { _errorMessage = e.message; _rawLlmResponse = e.rawResponse; _phase = _Phase.error; });
    } catch (e) {
      if (mounted) setState(() { _errorMessage = e.toString(); _phase = _Phase.error; });
    }
  }

  Future<void> _handleData(MesoImportData data, {String emptyMessage = 'No training days could be parsed. Check the format and try again.'}) async {
    if (data.days.isEmpty) {
      if (mounted) setState(() { _errorMessage = emptyMessage; _phase = _Phase.error; });
      return;
    }
    await _matchExercises(data);
    if (mounted) {
      _nameController.text = data.name;
      setState(() { _data = data; _phase = _Phase.review; });
    }
  }
  ...
  Future<void> _runLlm() async {
    ...
    try {
      ...
      await _handleData(data, emptyMessage: 'No training days could be parsed from the CSV. Check the format and try again.');
    } on LlmException catch (e) {
      setState(() {
